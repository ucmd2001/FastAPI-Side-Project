...
async def calculate_cart_total(data: CheckoutInput, service: PlanCartService = Depends()):
    try:
        parsed_data = await service.data_precheck(data.data)
        result = await service.calculate_cart_total(parsed_data)
        return {"total_price": str(result)}
